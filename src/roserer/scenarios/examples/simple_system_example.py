...
timer_callback = node_1.add_callback(wcet=1, name="cb_1", publishers=[pub_1])
timer_1 = node_1.add_timer(period=5, callback=timer_callback)

# Node 2
node_2 = executor.add_node(name="node_2")
x = node_2.add_variable(name="x")
subscription_callback = node_2.add_callback(wcet=1,write_variables=[x])
subscription_1 = node_2.add_subscription(name="sub_1", callback=subscription_callback, topic="topic_1")
service_callback = node_2.add_callback(wcet=1, name="serv_cb", read_variables=[x])
...
timer_2_callback = node_3.add_callback(wcet=1, name="timer_2_callback", request=ros.Request(client="client_1", response="response_cb"))
response_cb = node_3.add_callback(wcet=1, name="response_cb", write_variables=[y])
