...
def test_transform_system_service_client_correct_mapping() -> None:
    ...
    assert len(dust_sys.topics) == 2
    assert len(dust_sys.callbacks) == 3
    expected_request_cb = ds.PeriodicCallback(id=0,exec_time=3, period=5, type=0, offset=0, buffersize=1,amount_of_publishers=1,publisher_release_time=[3], publisher_id=[0], executorID=0, name='timer_1')
    expected_service_cb = ds.DataCallback(id=0, exec_time=7, topicID=0, type=1, buffersize=10,amount_of_publishers=1,publisher_release_time=[5], publisher_id=[1], executorID=1, name='service_1')
    expected_client_cb = ds.DataCallback(id=0,exec_time=1, topicID=1, type=3, buffersize=10,amount_of_publishers=0,publisher_release_time=[], publisher_id=[], executorID=0, name='client_1')
...
def test_transform_system_nested_calls_collapsed() -> None:
    ...
    expected_components.append(ds.PeriodicCallback(id=1, exec_time=9, period=10, type=0, offset=0,buffersize=1,amount_of_publishers=2,publisher_release_time=[4,9],publisher_id=[1,2],executorID=0, name='timer_2').__dict__) 
    expected_components.append(ds.DataCallback(id=0,exec_time=5,topicID=0,type=2,buffersize=10,amount_of_publishers=0,publisher_release_time=[],publisher_id=[],executorID=1, name='sub_1').__dict__)
    expected_components.append(ds.DataCallback(id=1,exec_time=5,topicID=1,type=2,buffersize=10,amount_of_publishers=0,publisher_release_time=[],publisher_id=[],executorID=1, name='sub_2').__dict__)
    expected_components.append(ds.DataCallback(id=2,exec_time=5,topicID=1,type=2,buffersize=10,amount_of_publishers=0,publisher_release_time=[],publisher_id=[],executorID=1, name='sub_3').__dict__)
    component_attributes = [com.__dict__ for com in dust_sys.callbacks + dust_sys.topics]
